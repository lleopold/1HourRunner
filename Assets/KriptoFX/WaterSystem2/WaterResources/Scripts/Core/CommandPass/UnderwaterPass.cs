...
namespace KWS
{
    internal class UnderwaterPass : WaterPass
    {
        ...
        private static readonly int                                 KwsUnderwaterNoiseBlurRadius = Shader.PropertyToID("KWS_UnderwaterNoiseBlurRadius");
        ...
        public override void ExecuteCommandBuffer(WaterPass.WaterPassContext waterContext)
        {
            ...
            if (WaterSystem.QualitySettings.DisableWaterDropsInScene && waterContext.cam.cameraType == CameraType.SceneView) return;
        ...
        public override void ExecuteBeforeCameraRendering(Camera cam, ScriptableRenderContext context)
        {
            ...
            if (WaterSystem.Instance.UseWaterDropsEffect) UpdateWaterDropsTimer(cam, 1.5f * WaterSystem.Instance.WaterDropsEffectTimeScale);
            if (WaterSystem.IsCameraPartialUnderwater || KWS_TileZoneManager.VisibleClipMeshZones.Count > 0) ExecuteInstanceBeforeCameraRendering(cam);
        }
        ...
        void ExecuteInstanceBeforeCameraRendering(Camera cam)
        {
            ...
            if (_underwaterMaterial == null) _underwaterMaterial       = KWS_CoreUtils.CreateMaterial(KWS_ShaderConstants.ShaderNames.UnderwaterShaderName, useWaterStencilMask: true);
            
            UpdateShaderParams(_underwaterMaterial);
            ...
            _renderParams.worldBounds        = new Bounds(Vector3.zero, 1000000 * Vector3.one);
        ...
        private void UpdateShaderParams(Material mat)
        {
            ...
            var noiseBlur                     = WaterSystem.Instance.UseUnderwaterNoiseBlur ? WaterSystem.Instance.UnderwaterNoiseBlurRadius : 0;
            
            mat.SetKeyword(KWS_ShaderConstants.WaterKeywords.USE_PHYSICAL_APPROXIMATION_COLOR, usePhysicalApproximationColor);
            mat.SetKeyword(KWS_ShaderConstants.WaterKeywords.USE_PHYSICAL_APPROXIMATION_SSR,   usePhysicalApproximationSSR);
            mat.SetKeyword(KWS_ShaderConstants.WaterKeywords.KWS_IS_CAMERA_PARTIAL_UNDERWATER, WaterSystem.IsCameraPartialUnderwater);
            
            mat.SetFloat(KwsUnderwaterNoiseBlurRadius, noiseBlur);

            mat.renderQueue = KWS_Settings.Water.DefaultWaterQueue + WaterSystem.QualitySettings.WaterTransparentSortingPriority + KWS_Settings.Water.UnderwaterQueueOffset;
        }
