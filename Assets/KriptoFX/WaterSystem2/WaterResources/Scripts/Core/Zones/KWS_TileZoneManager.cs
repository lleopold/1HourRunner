...
namespace KWS
{
    public class KWS_TileZoneManager
    {
        ...
        public static List<KWS_DynamicWavesSimulationEffector> DynamicWavesEffectors = new List<KWS_DynamicWavesSimulationEffector>();
        
        public static List<KWS_LocalWaterZone> VisibleCustomMeshZones = new List<KWS_LocalWaterZone>();
        public static List<KWS_LocalWaterZone> VisibleClipMeshZones = new List<KWS_LocalWaterZone>();
        ...
        public static List<KWS_DynamicWavesSimulationZone> VisibleBakedDynamicWavesZones =  new List<KWS_DynamicWavesSimulationZone>();

        public static Bounds VisibleDynamicWavesZonesBounds;
        
        internal static bool IsAnyZoneUseAdvectedUV;
        internal static bool IsAnyZoneUseRequireColorRendering;
        internal static bool IsAnyZoneUsePhytoplanktonEmission;
        internal static bool IsAnyZoneUseFoamParticles;
        ...
        public struct ZoneData
        {
            ...
            public uint    useAdvectedUV;
            public uint    useFoamTexture;
            public Vector4 foamData;
        ...
        public void ExecutePerFrame(HashSet<Camera> cameras)
        {
            ...
            VisibleBakedDynamicWavesZones.Clear();
            VisibleDynamicWavesZonesBounds.size = Vector3.zero;
            MaxZoneHeight                       = -1000000;
            
            if (WaterSystem.QualitySettings.UseDynamicWaves && DynamicWavesZones.Count > 0) UpdateDynamicWavesZonesVisibility(cam);
           
            
            VisibleCustomMeshZones.Clear();
            VisibleClipMeshZones.Clear();
        ...
        void UpdateDynamicWavesZonesVisibility(Camera cam)
        {
            foreach (var iZone in DynamicWavesZones)
            {
                ...
                if(zone.IsZoneInitialized == false) zone.ManualInitialize();
                
                iZone.UpdateVisibility(cam);
                ...
                {
                    case KWS_DynamicWavesSimulationZone.SimulationZoneTypeMode.StaticZone:
                        VisibleDynamicWavesZones.Add(iZone);
                        break;
                    case KWS_DynamicWavesSimulationZone.SimulationZoneTypeMode.MovableZone:
                        VisibleDynamicWavesZones.Add(iZone);
                        break;
                    case KWS_DynamicWavesSimulationZone.SimulationZoneTypeMode.BakedSimulation:
                        if (zone._isBakeMode || zone._isPreviewMode) VisibleDynamicWavesZones.Add(iZone);
                        VisibleBakedDynamicWavesZones.Add(zone);
                        break;
            ...
            IsAnyZoneUseAdvectedUV            = VisibleDynamicWavesZones.Cast<KWS_DynamicWavesSimulationZone>().Any(zone => zone.FoamType  == KWS_DynamicWavesSimulationZone.FoamTypeEnum.Advected);
            IsAnyZoneUseRequireColorRendering = VisibleDynamicWavesZones.Cast<KWS_DynamicWavesSimulationZone>().Any(zone => zone.RequireColorRendering);
            IsAnyZoneUseFoamParticles                  = VisibleDynamicWavesZones.Cast<KWS_DynamicWavesSimulationZone>().Any(zone => zone.UseFoamParticles);
            IsAnyZoneUsePhytoplanktonEmission = VisibleDynamicWavesZones.Cast<KWS_DynamicWavesSimulationZone>().Any(zone => zone.UsePhytoplanktonEmission);
            ...
            UpdateVisibleZoneIntersections();

            UpdateVisibleZonesBounds(VisibleDynamicWavesZones, out VisibleDynamicWavesZonesBounds);
            ...
            if (_lastVisibleDynamicWavesZonesHashCode != visibleZonesHashCode)
            {
                ...
                //UpdateWaterClusters(VisibleDynamicWavesZones, ref _tileIndexRangesBuffer_dynamicWaves, ref _globalTileIndicesBuffer_dynamicWaves, out VisibleDynamicWavesZonesBounds, out var gridSizeX, out var gridSizeZ);

                _lastVisibleDynamicWavesZonesHashCode = visibleZonesHashCode;

                if(_zoneDataBuffer_dynamicWaves != null && _zoneDataBuffer_dynamicWaves.count > 0) Shader.SetGlobalBuffer(KwsZoneData, _zoneDataBuffer_dynamicWaves);

                Shader.SetGlobalVector(WorldMin,  VisibleDynamicWavesZonesBounds.min);
                Shader.SetGlobalVector(WorldSize, VisibleDynamicWavesZonesBounds.size);
        ...
        void UpdateLocalWaterZonesVisibility(Camera cam)
        {
            foreach (var zone in LocalWaterZones)
            {
                ...
                var localZone = (KWS_LocalWaterZone)zone;
                if (localZone.OverrideMesh && localZone.CustomMesh)
                {
                    VisibleCustomMeshZones.Add(localZone);
                }

                if (localZone.UseClipMask && localZone.ClipMesh)
                {
                    VisibleClipMeshZones.Add(localZone);
                }

                if (localZone.AllOverridesDisabled() == false)
                {
                    VisibleLocalWaterZones.Add(zone);
                }
            }
            //Debug.Log("visible local zones " + VisibleLocalWaterZones.Count + "       visible mesh zones " + VisibleCustomMeshZones.Count);
        ...
        internal static void UpdateDynamicWavesZoneData(Vector3 cameraPosition)
        {
            ...
            for (int i = 0; i < VisibleDynamicWavesZones.Count; i++)
            {
                ...
                zone.ClosetsDistanceToCamera = (zone.OrientedBounds.ClosestPoint(cameraPosition) - cameraPosition).magnitude;
                ...
                _zoneData_dynamicWaves.Add(new ZoneData
                {
                    ...
                    particlesAlpha = unpackedZone.FoamParticlesAlphaMultiplier,
                    timeScale           = unpackedZone.CurrentTimeScale,
                    useAdvectedUV       = (uint)(unpackedZone.FoamType == KWS_DynamicWavesSimulationZone.FoamTypeEnum.Advected ? 1 : 0),
                    useFoamTexture      = (uint)(unpackedZone.UseFoamTexture ? 1 : 0),
                    foamData            = new Vector4((int)unpackedZone.FoamTextureType, unpackedZone.FoamTextureContrast, unpackedZone.FoamTextureScaleMultiplier, unpackedZone.FoamAlpha),
                    rainDropsIntensity = unpackedZone.RainDropsIntensity
                });
        ...
        static void UpdateVisibleZoneIntersections()
        {
            if(!KWS_DynamicWavesSimulationZone.IsAnyZoneRequireUpdateIntersections) return;
            
            foreach (var iZone in VisibleDynamicWavesZones)
            {
                var zone = (KWS_DynamicWavesSimulationZone)iZone;
                if(zone.ZoneType != KWS_DynamicWavesSimulationZone.SimulationZoneTypeMode.StaticZone) continue;
                
                zone.ForceUpdateZoneIntersections();
            }

            KWS_DynamicWavesSimulationZone.IsAnyZoneRequireUpdateIntersections = false;
        }

        static void UpdateVisibleZonesBounds(List<IWaterZone> zones, out Bounds bounds)
        {
            bounds = zones[0].OrientedBounds;
            for (int i = 1; i < zones.Count; i++)
            {
                if (zones[i] == null) continue;
                bounds.Encapsulate(zones[i].OrientedBounds);
            }
        }
        ...
        internal static void UpdateWaterClusters(List<IWaterZone> zones, ref ComputeBuffer tileIndexRangesBuffer, ref ComputeBuffer globalTileIndicesBuffer, out Bounds worldBounds, out int gridSizeX, out int gridSizeZ)
        {
           ...
           // stopwatch.Start();
            ...
            UpdateVisibleZonesBounds(zones, out worldBounds);
    ...
    internal class WaterZoneDistanceComparer : IComparer<KWS_TileZoneManager.IWaterZone>
    {
        ...
        {
            float distA = Vector3.SqrMagnitude(a.OrientedBounds.ClosestPoint(CameraPosition) - CameraPosition);
            float distB = Vector3.SqrMagnitude(b.OrientedBounds.ClosestPoint(CameraPosition) - CameraPosition);
