...
namespace KWS
{
    internal static partial class KWS_CoreUtils
    {
        ...
        public static GraphicsFormat GetGraphicsFormatHDR(bool falbackHDR = true)
        {
            ...
            else return SystemInfo.GetGraphicsFormat(falbackHDR ? DefaultFormat.HDR : DefaultFormat.LDR);
#else
        ...
        public static GraphicsFormat GetSafeR8G8B8A8_UNorm()
        {
        ...
        public static GraphicsFormat GetSafeR8_UNorm()
        {
        ...
        public static bool IsFrameDebuggerEnabled()
        {
            ...
            return focusedWindow != null && focusedWindow.titleContent.text == "Frame Debugger";
#else
            return false;
#endif
        }
        ...
        public static Material CreateMaterial(string shaderName, bool useWaterStencilMask = true)
        {
...
#if UNITY_PS4 || UNITY_PS5  
            waterMaterial.SetInt("_ZWriteOverride", 0);
#endif
            return waterMaterial;
        }

        public static Material CreateSerializedMaterial(UnityEngine.Object target, string shaderName, bool useWaterStencilMask = true)
        {
            var waterShader = Shader.Find(shaderName);
            if (waterShader == null)
            {
            ...
            if (comp && comp.gameObject.scene.IsValid())
            {
        ...
        public static  void SetFallbackBuffer<T>(ref ComputeBuffer buffer, int shaderNameID, Material mat) where T : struct
        {
            buffer = KWS_CoreUtils.GetOrUpdateBuffer<T>(ref buffer, 1);
            buffer.SetData(new T[1]);
            mat.SetBuffer(shaderNameID, buffer);
        }
        ...
        public static void SetViewport(this CommandBuffer cmd, UnityEngine.Rendering.RTHandle target)
        {
            if (target.useScaling)
            {
                var scaledViewportSize = target.GetScaledSize(target.rtHandleProperties.currentViewportSize);
        ...
        public static void SetViewportAndClear(this CommandBuffer cmd, UnityEngine.Rendering.RTHandle target, ClearFlag clearFlag, Color clearColor)
        {
#if !UNITY_EDITOR
            SetViewport(cmd, target);
#endif
        ...
        public static void BlitTriangleRTHandle(this CommandBuffer cmd, RTHandle source, UnityEngine.Rendering.RTHandle  target, Material mat, ClearFlag clearFlag, Color clearColor, int pass = 0)
        {
            cmd.SetGlobalVector(_sourceRTHandleScale_id, source.rtHandleProperties.rtHandleScale);
            cmd.SetGlobalTexture(_sourceRT_id, source);
            cmd.SetRenderTarget(target, 0, CubemapFace.Unknown, -1);
        ...
        public static void BlitTriangleRTHandle(this CommandBuffer cmd, RTHandle source, RenderTargetIdentifier  target, Material mat, ClearFlag clearFlag, Color clearColor, int pass = 0)
        {
            cmd.SetGlobalVector(_sourceRTHandleScale_id, source.rtHandleProperties.rtHandleScale);
            cmd.SetGlobalTexture(_sourceRT_id, source);
            cmd.SetRenderTarget(target, 0, CubemapFace.Unknown, -1);
        ...
        private static RenderTargetIdentifier[] mrt5       = new RenderTargetIdentifier[5];
        ...
        public static RenderTargetIdentifier[] GetMrt(RenderTargetIdentifier rt1, RenderTargetIdentifier rt2, RenderTargetIdentifier rt3, RenderTargetIdentifier rt4, RenderTargetIdentifier rt5)
        {
            mrt5[0] = rt1;
            mrt5[1] = rt2;
            mrt5[2] = rt3;
        ...
        public static void ComputeSDF(CommandBuffer cmd, Material sdfMaterial, float maxAreaSize, Vector4 bakedNearFarSizeXZ, float waterPos, RenderTexture depthSource, RenderTexture sdfTarget)
        {
            ...
            int tempRt1      = Shader.PropertyToID("KWS_SdfTemp1");
            int tempRt2      = Shader.PropertyToID("KWS_SdfTemp2");
            int tempPackedRt = Shader.PropertyToID("KWS_SdfPackedTemp");
            
            cmd.GetTemporaryRT(tempRt1,      sdfTarget.width, sdfTarget.height, 0, FilterMode.Point,    GraphicsFormat.R32G32_SFloat);
            cmd.GetTemporaryRT(tempRt2,      sdfTarget.width, sdfTarget.height, 0, FilterMode.Point,    GraphicsFormat.R32G32_SFloat);
            cmd.GetTemporaryRT(tempPackedRt, sdfTarget.width, sdfTarget.height, 0, FilterMode.Bilinear, GraphicsFormat.R32G32B32A32_SFloat);
            ...
            cmd.SetGlobalFloat("SDF_ShoreThreshold", 0.5f);

            // pass 0 - seeds
            cmd.BlitTriangle(sdfMaterial, tempRt1, 0);

            var stepSize = new Vector2(sdfTarget.width, sdfTarget.height);
            int passes   = Mathf.CeilToInt(Mathf.Log(Mathf.Max(sdfTarget.width, sdfTarget.height), 2.0f));
            ...
            for (int i = 0; i < passes; i++)
            {
                ...
                cmd.BlitTriangle(source, target, sdfMaterial, 1);

                (source, target) = (target, source);
            }

            // pass 2 - finalize packed sdf
            cmd.BlitTriangle(source, tempPackedRt, sdfMaterial, 2);

            cmd.BlitTriangle(tempPackedRt, sdfTarget, sdfMaterial, 3);
            
            cmd.ReleaseTemporaryRT(tempRt1);
            cmd.ReleaseTemporaryRT(tempRt2);
            cmd.ReleaseTemporaryRT(tempPackedRt);
