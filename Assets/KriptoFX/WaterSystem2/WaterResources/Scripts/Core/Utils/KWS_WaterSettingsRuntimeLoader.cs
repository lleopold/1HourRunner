...
namespace KWS
{
    public static class KWS_WaterSettingsRuntimeLoader
    {
        ...
        static void LoadEditorWaterSettings()
        {
            ...
            if (_waterSystemQualitySettings == null || _waterQualityLevelSettings == null || _waterSystemQualitySettings.qualityLevelSettings.Count == 0 || _waterSystemQualitySettings.qualityLevelSettings.Count != QualitySettings.names.Length)
            {
...
//         }
        ...
        internal static void GetOrCreateSettings()
        {
            var resourcesFolderAssetPath = KW_Extensions.GetResourcesFolderAssetPath();
            if (string.IsNullOrEmpty(resourcesFolderAssetPath)) return;

            var pathToSettingsFile = $"{resourcesFolderAssetPath}/{settingsName}";
        ...
        public static void SyncDefualtQualitySettings(WaterQualityLevelSettings settings, string level)
        {
            ...
            settings.OceanCausticTextureResolutionQuality   = KW_Extensions.StringToEnum(level, WaterQualityLevelSettings.CausticTextureResolutionQualityEnum.High);
