...
namespace KWS
{
    internal class MeshQuadTree
    {
        ...
        internal class QuadTreeInstance : KW_Extensions.ICacheCamera
        {
            ...
            internal HashSet<Node> VisibleZoneNodes    = new HashSet<Node>();
            ...
            public   float       _lastOceanLevel = float.PositiveInfinity;
            ...
            public void Release()
            {
                ...
                _lastOceanLevel     = float.PositiveInfinity;
            }
        }
        ...
        public bool TryGetRenderingContext(Camera cam, int lodBias, out QuadTreeRenderingContext context)
        {
#if KWS_DEBUG
            if ((KWS_Ocean.Instance != null && KWS_Ocean.Instance.DebugQuadtree)) cam = Camera.main;
#endif
            if (cam != null && Instances.TryGetValue(cam, out var instance) && instance.CanRender)
            {
                var level = Mathf.Clamp(instance.ActiveMeshDetailingInstanceIndex - lodBias, 0, InstanceMeshes.Count - 1);
        ...
        {
            var farDist = WaterSystem.QualitySettings.MeshDetailingFarDistance * WaterSystem._globalWaterScaleFactor;
        ...
        public void Initialize(WaterSystem waterInstance)
        {
            ...
            _maxWavesHeight = waterInstance.CurrentMaxOceanWaveHeight;
        ...
        public void UpdateQuadTree(Camera cam, WaterSystem waterInstance, bool forceUpdate = false)
        {
            ...
            if (KWS_TileZoneManager.VisibleDynamicWavesZones.Count > 0 || KWS_TileZoneManager.VisibleLocalWaterZones.Count > 0) _maxDynamicZoneHeightOffset = KWS_TileZoneManager.MaxZoneHeight - waterInstance.WaterLevel;
            ...
            instance.VisibleZoneNodes.Clear();

            _waterLevel                = waterInstance.WaterLevel;
            _maxWavesHeight            = waterInstance.CurrentMaxOceanWaveHeight;
            _wideAngleMode             = WaterSystem.QualitySettings.WideAngleCameraRenderingMode;
            ...
            camOffset.y = _waterLevel;
            var halfOffset = _quadTreeBounds.size.y * 0.5f;

            if (KWS_Ocean.Instance == false)
            {
                var visibleZones = KWS_TileZoneManager.VisibleDynamicWavesZones;
                
                foreach (var visibleNode in instance.VisibleNodes)
                {
                    foreach (var iZone in visibleZones)
                    {
                        var zone                    = (KWS_DynamicWavesSimulationZone)iZone;
                        var updateDistanceThreshold = KWS_Settings.Mesh.UpdateQuadtreeEveryMetersForward;
                        var nodeBounds              = new Bounds(visibleNode.ChunkCenter + camOffset + new Vector3(0, halfOffset, 0), visibleNode.ChunkSize + new Vector3(updateDistanceThreshold, 0, updateDistanceThreshold));
                        if (zone.ZoneType != KWS_DynamicWavesSimulationZone.SimulationZoneTypeMode.BakedSimulation && iZone.OrientedBounds.Intersects(nodeBounds))
                        {
                            instance.VisibleZoneNodes.Add(visibleNode);
                        }
                    }
                }
            }
            // else
            // {
            //     var visibleBakedZones = KWS_TileZoneManager.VisibleBakedDynamicWavesZones;
            //     
            //     foreach (var visibleNode in instance.VisibleNodes)
            //     {
            //         foreach (var zone in visibleBakedZones)
            //         {
            //             var updateDistanceThreshold = KWS_Settings.Mesh.UpdateQuadtreeEveryMetersForward;
            //             var nodeBounds              = new Bounds(visibleNode.ChunkCenter + camOffset + new Vector3(0, halfOffset, 0), visibleNode.ChunkSize - new Vector3(updateDistanceThreshold, 0, updateDistanceThreshold));
            //             var zoneBounds              = ((KWS_TileZoneManager.IWaterZone)zone).OrientedBounds;
            //             zoneBounds.size *= 0.5f;
            //             if (zoneBounds.Intersects(nodeBounds) == false)
            //             {
            //                 instance.VisibleZoneNodes.Add(visibleNode);
            //             }
            //         }
            ...
            var visibleNodes = KWS_Ocean.Instance != null ? instance.VisibleNodes : instance.VisibleZoneNodes;
            ...
            instance._lastOceanLevel             = waterInstance.WaterLevel;
            ...
            UpdateQuadTreeDetailingRelativeToWind(instance, KWS_Ocean.Instance != null ? KWS_Ocean.Instance.WindSpeed : 0);
        ...
        bool IsRequireUpdateQuadTree(QuadTreeInstance instance, WaterSystem waterInstance)
        {
            ...
            if (KW_Extensions.SqrMagnitudeFast(instance._camForward - instance._lastCameraForward) > KWS_Settings.Mesh.QuadtreeRotationThresholdForUpdate            ||
                ...
                Mathf.Abs(instance._lastOceanLevel - waterInstance.WaterLevel)                > 0.001f                                                               ||
                Math.Abs(_maxWavesHeight           - waterInstance.CurrentMaxOceanWaveHeight) > 0.001f                                                               ||
        ...
        public void UpdateQuadTreeDetailingRelativeToWind(QuadTreeInstance instance, float windSpeed)
        {
            var lodOffset      = KWS_TileZoneManager.VisibleDynamicWavesZones.Count > 0 ? KWS_Settings.Water.QuadTreeChunkLodOffsetForDynamicWaves : 0;
        ...
        internal class Node
        {
            ...
            internal ChunkVisibilityEnum UpdateVisibleNodes(QuadTreeInstance instance, MeshQuadTree root)
            {
                ...
                currentCenter.y = root._waterLevel - ChunkSize.y * 0.5f;
                ...
                var surfaceHeight    = root._waterLevel + root._maxWavesHeight;
                var distanceToCamera = Mathf.Abs(surfaceHeight - instance._camPos.y);

                if (KWS_TileZoneManager.VisibleDynamicWavesZones.Count > 0)
                {
                    float zoneMaxQualityLevelOffset = 20;
                    float zoneSurfaceHeight         = root._waterLevel + Mathf.Max(root._maxWavesHeight, root._maxDynamicZoneHeightOffset) + zoneMaxQualityLevelOffset;
