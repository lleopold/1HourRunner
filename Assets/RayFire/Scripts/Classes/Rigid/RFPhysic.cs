    ...
    public class RFPhysic
    {
        ...
        public static void SetFragmentsVelocity (RayfireRigid scr)
        {
            ...
            else
            {
                ...
                if (scr.simTp != SimType.Static && scr.objTp != ObjectType.SkinnedMesh)
                    baseVelocity = scr.physics.rb.linearVelocity * scr.physics.dm;
                ...
                for (int i = 0; i < scr.fragments.Count; i++)
                {
                    if (scr.fragments[i] != null)
                        ...
                        {
                            if (scr.fragments[i].objTp == ObjectType.Mesh)
                            {
                                scr.fragments[i].physics.rb.linearVelocity = baseVelocity;
                            }
                            else
                            {
                                SetClusterVelocity (scr.fragments[i].physics, scr.fragments[i].physics, scr.physics.dm);
                            }
                        }
                }
            }
        ...
        public static void SetClusterVelocity(RFPhysic clsPh, RFPhysic sourcePh, float mult = 1f)
        {
            //Debug.Log (sourcePh.velocity * mult);
            clsPh.rb.linearVelocity = sourcePh.velocity * mult;
            //Debug.Log (clsPh.rb.velocity);
        }
