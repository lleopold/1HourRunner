...
namespace RayFire
{
    ...
    public class RFDemolitionCluster
    {
        ...
        // UI
        public ConnectivityType cnt;
        public FragSimType      sim;
        public float            mAr; // minimumArea
        public float            mSz; // minimumSize
        public int              pct; // percentage
        public int              seed;
        public RFDetachType     type;
        public int              ratio;
        public float            units;
        public int              sAr;  // shardArea
        public bool             sDm;  // shardDemolition
        public int              mnAm; // minAmount
        public int              mxAm; // maxAmount
        public bool             cDm;  // demolishable
        public RFCollapse       collapse;
        public int              clsCount;
        public RFCluster        cluster;
        public List<RFCluster>  minorClusters;
        public bool             cn; // show connection
        public bool             nd; // show nodes
        ...
        static void ClusterizeConnected (RayfireRigid scr)
        {
            if (scr.clsDemol.cluster.shards.Count == 0)
            {
                ...
                // Set connections amount
                scr.clsDemol.cluster.cnAm = 0;
                for (int s = 0; s < scr.clsDemol.cluster.shards.Count; s++)
                    scr.clsDemol.cluster.cnAm += scr.clsDemol.cluster.shards[s].neibShards.Count;
            }
        }
        ...
        static void UpdateOriginalCluster (RayfireRigid scr)
        {
            ...
            // RFPhysic.SetClusterVelocity (scr.physics, scr.physics, scr.physics.dm);
        }
        ...
        static void ClusterizeDetachShards (RayfireRigid scr, List<RFShard> detachShards, int clusterAmount, int sortType)
        {
            ...
            // Original to child cluster check
            DuplicateChildClusterCheck (scr, detachShards);
        ...
        // Original to child cluster check
        static void DuplicateChildClusterCheck(RayfireRigid scr, List<RFShard> detachShards)
        {
            if (scr.clsDemol.cluster.shards.Count == 0 && detachShards.Count == 0 && scr.clsDemol.cluster.childClusters.Count == 1)
            {
                for (int i = 0; i < scr.clsDemol.cluster.childClusters[0].shards.Count; i++)
                {
                    scr.clsDemol.cluster.childClusters[0].shards[i].cluster = null;
                    detachShards.Add (scr.clsDemol.cluster.childClusters[0].shards[i]);
                }
                scr.clsDemol.cluster.childClusters.Clear();
            }
        }
        ...
        public static void CreateClusterRuntime (RayfireRigid scr, RFCluster cluster)
        {
            ...
            if (scr != cluster.rigid)
            {
                ...
                // Save original cluster velocity
                cluster.rigid.physics.velocity = scr.physics.velocity;
            ...
            // Set saved velocity
            // RFPhysic.SetClusterVelocity (cluster.rigid.physics, scr.physics, scr.physics.dm);
