...
namespace RayFire
{
    ...
    public class RFFadeBatch
    {
        ...
        static void StartFadeRigid(RFFadeBatch batch)
        {
            ...
            else if (batch.fadeType == FadeType.FallDown && batch.rigid.gameObject.activeSelf == true)
                batch.rigid.StartCoroutine (RFFade.FallDownCor (batch.rigid));
            else if (batch.fadeType == FadeType.ScaleDown && batch.rigid.gameObject.activeSelf == true)
                batch.rigid.StartCoroutine (RFFade.ScaleDownCor (batch.rigid));
            else if (batch.fadeType == FadeType.MoveDown && batch.rigid.gameObject.activeSelf == true)
        ...
        // Set rigidbody to kinematiс
        static void SetKinematiс(RayfireRigid scr)
        {
            scr.fading.state           = 3;
