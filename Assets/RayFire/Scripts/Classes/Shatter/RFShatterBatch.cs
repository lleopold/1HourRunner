    ...
    public class RFShatterBatch
    {
        ...
        public void SaveData(RayfireShatter scr)
        {
            ...
            else if (scr.type == FragType.Radial)
            {
                ...
                dataFrg.Add (0); // Obsolete focus str
        ...
        {
            RFShatterBatch batch = new RFShatterBatch(sh.transform, sh.engine.mainRoot.transform);
            batch.SaveData (sh);
            batch.skin = sh.engine.HasSkin;
        ...
        // Destroy all batches
        public static void DestroyBatches(RayfireShatter scr)
        {
            if (scr.HasBatches == true)
            {
                for (int i = scr.batches.Count - 1; i >= 0; i--)
                    if (scr.batches[i].fragRoot != null)
                        Object.DestroyImmediate (scr.batches[i].fragRoot.gameObject);
                scr.batches.Clear();
            }
        } 
