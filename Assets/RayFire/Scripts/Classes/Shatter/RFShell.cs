...
namespace RayFire
{
    ...
    public class RFShell
    {
        ...
        public static float minThickness = 0.002f;
        ...
        public static Mesh AddShell(Utils.Mesh utilsMesh, Mesh unityMesh, bool bridge, bool mergeSubMeshes, float thickness)
        {
            // No thickness
            if (thickness <= 0)
                return unityMesh;
            ...
            {
                RayfireMan.Log (RFLog.shl_dbgn + unityMesh.name + RFLog.shl_noEdg);
                return unityMesh;
            }
            ...
            return CombineMesh(unityMesh, shellMesh, bridgeMesh, mergeSubMeshes); 
        }
        ...
        static Mesh CombineMesh(Mesh outerMesh, Mesh innerMesh, Mesh bridgeMesh, bool mergeSubMeshes)
        {
            ...
            // Get mesh index format
            IndexFormat indexFormat = IndexFormat.UInt16;
            int         verts       = 0;
            for (int i = 0; i < combine.Length; i++)
                verts += combine[i].mesh.vertexCount;
            if (verts > 65535)
