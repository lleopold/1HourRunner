...
namespace RayFireEditor
{
    ...
    public class RayfireShatterEditor : Editor
    {
        ...
        const int   adv_element_min = 0;
        const int   adv_element_max = 99;
        ...
        SerializedProperty sp_tp_tet_lat;
        SerializedProperty sp_tp_tet_dns;
        SerializedProperty sp_tp_tet_nse;
        ...
        private void OnEnable()
        {
            ...
            sp_tp_tet_lat      = serializedObject.FindProperty(nameof(shatter.tets) + "." + nameof(shatter.tets.lattice));
            sp_tp_tet_dns      = serializedObject.FindProperty(nameof(shatter.tets) + "." + nameof(shatter.tets.density));
            sp_tp_tet_nse      = serializedObject.FindProperty(nameof(shatter.tets) + "." + nameof(shatter.tets.noise));
        ...
        void GUI_Batches()
        {
            ...
            for (int i = 0; i < shatter.batches.Count; i++)
            {
                ...
                if (GUILayout.Button (TextSht.str_delete, RFUI.buttonStyle, GUILayout.Height (18), GUILayout.Width (60)))
                {
        ...
        void GUI_Part()
        {
            ...
            if (EditorGUI.EndChangeCheck() == true)
            {
                //CheckPropertyChange(shatter.advanced.ab_obj, sp_aabb_obj);
                InteractiveChange();
            }
        }
        ...
        void CheckPropertyChange(Transform tm, SerializedProperty prop)
        {
            ...
            // Disabled for partial fragmentation because of Unity UI crash
            if (sp_aabb_en.boolValue == true)
                return;
