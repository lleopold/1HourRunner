...
namespace RayFireEditor
{
    ...
    public class RayfireRigidEditor : Editor
    {
        ...
        const int   cls_frg_min        = 0;
        const int   cls_frg_max        = 60;
        ...
        private void OnEnable()
        {
            ...
            sp_act_l   = serializedObject.FindProperty(nameof(rigid.act) + "." + nameof(rigid.act.len));
            ...
            sp_msh_cls     = serializedObject.FindProperty(nameof(rigid.mshDemol) + "." + nameof(rigid.mshDemol.cls));
        ...
        void GUI_Main_Warning()
        {
            ...
            if (sp_mn_obj.intValue == (int)ObjectType.SkinnedMesh)
            ...
            if (MeshState() == true && rigid.dmlTp != DemolitionType.None)
            {
                ...
                if (fld_msh == true)
                {
                    ...
                    if (rigid.mshDemol.use == false)
                    {
                        ...
                        RFUI.IntSlider (sp_msh_cls, cls_frg_min,  cls_frg_max,  TextMsh.gui_msh_cls);
        ...
        void GUI_Info()
        {
            ...
            if (Application.isPlaying == true && rigid.enabled == true && rigid.initialized == true && rigid.objTp != ObjectType.MeshRoot)
            {
                ...
                if (rigid.mshDemol.ch.tp != CachingType.Disabled)
                {
                    if (rigid.mshDemol.ch.inProgress == true)
                        GUILayout.Label (TextRig.str_ch_prog);
                    else if (rigid.mshDemol.ch.wasUsed == true)
                        GUILayout.Label (TextRig.str_ch_used);
                }
                
                // Awake precache
                if (rigid.dmlTp == DemolitionType.AwakePrecache)
                    if (rigid.mshDemol.HasEngineAndMeshes == true)
                        GUILayout.Label (TextRig.str_precache);
            ...
            if (rigid.objTp == ObjectType.NestedCluster || rigid.objTp == ObjectType.ConnectedCluster)
            {
                ...
                if (rigid.objTp == ObjectType.ConnectedCluster)
                {
                    ...
                    GUILayout.Label (TextRig.str_conns + rigid.clsDemol.cluster.cnAm / 2);
                }
